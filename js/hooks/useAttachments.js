...
const reducer = (state, action) => {
    ...
    switch (action.type) {
        case ActionType.ADD_ATTACHMENT: {
            const newAllPageAttachmentsAdd = allPageAttachments.map((attachments, index) => pageIndex === index
        ...
        case ActionType.REMOVE_ATTACHMENT: {
            const newAllPageAttachmentsRemove = allPageAttachments.map((otherPageAttachments, index) => pageIndex === index
        ...
        case ActionType.UPDATE_ATTACHMENT: {
            ...
            const newAllPageAttachmentsUpdate = allPageAttachments.map((otherPageAttachments, index) => pageIndex === index
                ? pageAttachments.map((oldAttachment, _attachmentIndex) => _attachmentIndex === action.attachmentIndex
...
export const useAttachments = () => {
    ...
    const add = (newAttachment) => dispatch({ type: ActionType.ADD_ATTACHMENT, attachment: newAttachment });
    const remove = (attachmentIndex) => dispatch({ type: ActionType.REMOVE_ATTACHMENT, attachmentIndex });
    const update = (attachmentIndex, attachment) => dispatch({
        type: ActionType.UPDATE_ATTACHMENT,
        attachmentIndex,
        attachment,
    });
    const reset = (numberOfPages) => dispatch({ type: ActionType.RESET, numberOfPages });
    const setPageIndex = useCallback((index) => dispatch({ type: ActionType.UPDATE_PAGE_INDEX, pageIndex: index }), [dispatch]);
    return {
        add,
        reset,
        remove,
