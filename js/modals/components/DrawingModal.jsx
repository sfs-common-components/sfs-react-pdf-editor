...
export const DrawingModal = ({ open, dismiss, confirm, drawing }) => {
    ...
    return (<Modal size="small" dimmer="inverted" open={open} onClose={closeModal}>
        ...
        <Menu size="tiny">
          ...
          <Menu.Menu position="right">
            <Dropdown item text={`${strokeWidth}`}>
              <Dropdown.Menu>
                {strokeSizes.map((size) => (<Dropdown.Item key={size} selected={size === strokeWidth} onClick={() => setStrokeWidth(size)}>
                    {size}
                  </Dropdown.Item>))}
              </Dropdown.Menu>
            </Dropdown>
            ...
            padding: 5,
                  ...
                  {Object.values(Color).map((color, index) => (<div style={{ margin: 2.5 }} key={index}>
      ...
      <Modal.Actions>
        ...
        <Button content="Done" labelPosition="right" icon="checkmark" onClick={handleDone} positive/>
      </Modal.Actions>
    </Modal>);
};
